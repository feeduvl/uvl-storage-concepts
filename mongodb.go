...
const (
	...
	collectionAppReviewCrawlerJobs = "crawler_jobs_for_reviews"
...
func MongoGetAppReviewCrawlerJobs(mongoClient *mgo.Session) [] AppReviewCrawlerJobs {
	...
	err := mongoClient.
		DB(database).
		C(collectionAppReviewCrawlerJobs).Find(bson.M{}).Select(bson.M{"app_name": 1, "app_url": 1, "date": 1, "occurrence": 1, "number_posts": 1, "dataset_name": 1, "request": 1}).All(&crawlerJobs)
